...
function OtpPopUp(props) {
  ...
  async function handleResendOtp() {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/user/registerUser`,
        props.completeData
      );
      if (response.data.message) {
    ...
    <div className="w-screen h-screen fixed top-0 left-0 backdrop-blur-[3px] flex justify-center items-center poppins-font">
      <div className="w-[90%] sm:w-[500px] max-w-[500px] h-[280px] pb-5 rounded-2xl bg-white popup-button-shadow">
        ...
        <div className="w-full h-1/2 flex flex-col justify-center items-center gap-0">
          <div className="w-full pt-2 flex justify-center gap-8">
            {Array.from({ length: 4 }, (_, index) => (
              <input
                ...
                ref={inputRefs[index]}
                className={
                  "text-center size-10 border-2 border-black rounded-lg"
                }
                maxLength="1"
          ...
          <span className="mt-4 text-green-600 font-semibold text-sm sm:text-base">
            {timer}
          </span>
          {isInvalidOtp && (
            <span className="mt-2 text-xs sm:text-sm text-[#FF0000]">
              {otpErrorMessage}
            </span>
          )}
        </div>
          ...
          <button
            className={
              isResendButtonEnabled
                ? "w-20 sm:w-24 py-2 text-white font-semibold text-xs sm:text-sm bg-gradient-to-b from-[#5A5A5A] to-[#000000] hover:to-[#000000bc] rounded-xl popup-button-shadow"
